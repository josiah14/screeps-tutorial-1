...
var creepActions = ['drop', 'transfer', 'withdraw', 'pickup', 'heal', 'rangedHeal', 'dismantle', 'attack', 'harvest', 'move', 'repair', 'build', 'rangedMassAttack', 'rangedAttack', 'suicide', 'say', 'claimController', 'upgradeController', 'reserveController', 'attackController', 'generateSafeMode', 'signController'];

var modules = require('bulk-require')(__dirname, ['*.js']);

function checkPriorities(intents, name) {
    return intents[name] && (!priorities[name] || !_.any(priorities[name], function (i) {
        return !!intents[i];
    }));
}
...
module.exports = function (object, objectIntents, roomObjects, roomTerrain, bulk, bulkUsers, roomController, stats, gameTime, roomInfo) {
    creepActions.forEach(function (name) {
