...
module.exports = function (object, intent, roomObjects, roomTerrain, bulk, bulkUsers, roomController, stats, gameTime) {
    ...
    if (_.contains(C.OBSTACLE_OBJECT_TYPES, target.structureType) && _.any(roomObjects, function (i) {
        return i.x == target.x && i.y == target.y && _.contains(C.OBSTACLE_OBJECT_TYPES, i.type);
    })) {
        return;
    }
    ...
    var buildPower = _.filter(object.body, function (i) {
        return (i.hits > 0 || i._oldHits > 0) && i.type == C.WORK;
    }).length * C.BUILD_POWER || 0,
        buildRemaining = target.progressTotal - target.progress,
        buildEffect = Math.min(buildPower, buildRemaining, object.energy),
        boostedParts = _.map(object.body, function (i) {
    ...
    boostedParts.sort(function (a, b) {
        return b - a;
    });
