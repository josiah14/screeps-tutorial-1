...
exports.make = function (runtimeData, intents, register) {
    ...
    var market = {
        ...
        createOrder: register.wrapFn(function (type, resourceType, price, totalAmount, roomName) {
            ...
            if (resourceType != C.SUBSCRIPTION_TOKEN && (!roomName || !_.any(runtimeData.userObjects, { type: 'terminal', room: roomName }))) {
            ...
            intents.pushByName('market', 'createOrder', {
                type: type, resourceType: resourceType, price: price, totalAmount: totalAmount, roomName: roomName
            });
            return C.OK;
        }),
        ...
        cancelOrder: register.wrapFn(function (orderId) {
            ...
            intents.pushByName('market', 'cancelOrder', { orderId: orderId }, 50);
            return C.OK;
        }),
        ...
        deal: register.wrapFn(function (orderId, amount, targetRoomName) {
            ...
            if (order.resourceType == C.SUBSCRIPTION_TOKEN) {
                ...
                if (order.type == C.ORDER_BUY) {
                    if (order.resourceType != C.RESOURCE_ENERGY && (!terminal[order.resourceType] || terminal[order.resourceType] < amount) || order.resourceType == C.RESOURCE_ENERGY && terminal[C.RESOURCE_ENERGY] < amount + transferCost) {
                        return C.ERR_NOT_ENOUGH_RESOURCES;
                    }
                }
            }
            ...
            if (!intents.pushByName('market', 'deal', { orderId: orderId, targetRoomName: targetRoomName, amount: amount }, 10)) {
                return C.ERR_FULL;
            }
            return C.OK;
        }),
        ...
        changeOrderPrice: register.wrapFn(function (orderId, newPrice) {
            ...
            intents.pushByName('market', 'changeOrderPrice', {
                orderId: orderId, newPrice: newPrice
            }, 50);
            return C.OK;
        }),
        ...
        extendOrder: register.wrapFn(function (orderId, addAmount) {
            ...
            intents.pushByName('market', 'extendOrder', {
                orderId: orderId, addAmount: addAmount
            }, 50);
            return C.OK;
        })
    };
    ...
    Object.defineProperties(market, {
        ...
        incomingTransactions: {
            ...
            get: function get() {
                if (!_incomingTransactions) {
                    _incomingTransactions = _.map(runtimeData.transactions.incoming || [], function (i) {
        ...
        outgoingTransactions: {
            ...
            get: function get() {
                if (!_outgoingTransactions) {
                    _outgoingTransactions = _.map(runtimeData.transactions.outgoing || [], function (i) {
                        i.transactionId = "" + i._id;
                        delete i._id;
                        i.sender = i.sender ? { username: runtimeData.users[i.sender].username } : undefined;
                        i.recipient = i.recipient ? { username: runtimeData.users[i.recipient].username } : undefined;
        ...
        orders: {
            ...
            get: function get() {
                if (!_orders) {
                    _orders = _(runtimeData.market.myOrders).map(function (i) {
                        i.id = "" + i._id;
                        delete i._id;
                        delete i.user;
                        return i;
